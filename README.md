...
- 本リポジトリは著者が運営しています。出版社は本リポジトリの運営・内容には関与していません。本リポジトリに関する問い合わせを出版社へ送らないでください。
- 本リポジトリのコードは、書籍の内容を理解するための**学習用サンプル**です。エラー処理・認証・セキュリティ対策は説明のために簡略化しており、そのまま本番環境で使用することは想定していません。
- コードは現状のまま（as-is）提供され、動作・正確性・特定目的への適合性についていかなる保証もしません。利用により生じた損害について、著者および出版社は責任を負いません。
- `ANTHROPIC_API_KEY` を使った実行では、読者ご自身のアカウントに API 利用料金が発生します。
- モデル名・ライブラリのバージョン・外部サービス（Anthropic API、Dify、promptfoo 等）の画面や仕様は執筆時点（2026年）のものです。変更により手順どおり動かない場合は、各サービスの公式ドキュメントを確認してください。
- 本リポジトリでは **Issue / Pull Request を受け付けていません**。個別のサポートも行いません。書籍本文の正誤については、出版社の書籍サポートページをご確認ください。
