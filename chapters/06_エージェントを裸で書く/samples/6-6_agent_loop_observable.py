"""6-5 / 6-6 停止条件とログ・観測性を入れた手書きエージェント。

本書 6-5（停止条件）と 6-6（ログと観測性）の掲載コードを、ひとつの実行できる
スクリプトにまとめたもの。6-4 の最小ループに対して、次の2つを足している。

  - 6-5: while True を for step in range(MAX_STEPS) に置き換え、上限で必ず止める。
         for ... else で「break せずに回り切った＝上限到達」を検知して警告する。
  - 6-6: 各ステップに print ログを仕込み、stop_reason・トークン・道具呼び出しを記録する。

ツール（get_stock）とモデル名は 6-4 のスクリプトから読み込んで再利用する
...
1. 既定（APIキー不要）:
   ...
   シミュレートして表示する。本文 6-6 の「期待される記録」と同じ形を再現する。
...
実行:
    ...
    ANTHROPIC_API_KEY=sk-... python 6-6_agent_loop_observable.py   # 本番

注意:
    モデル名は 6-4 の MODEL を共有する。新しい世代が出たら 6-4 側を更新すれば
...
# 6-4 のスクリプトからツール定義・実装・モデル名を再利用する。
# ファイル名が数字始まりで通常の import が使えないため importlib を使う。
_base = importlib.import_module("6-4_agent_loop_minimal")
MODEL = _base.MODEL
TOOLS = _base.TOOLS
...
def run_dry(task: str) -> None:
    """APIキーなしのドライラン。本文 6-6 と同じ形のステップログを再現する。"""
