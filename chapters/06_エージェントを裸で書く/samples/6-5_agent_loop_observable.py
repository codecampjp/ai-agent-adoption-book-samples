"""6-4 / 6-5 停止条件とログ・観測性を入れた手書きエージェント。

本書 6-4（停止条件）と 6-5（ログと観測性）の掲載コードを、ひとつの実行できる
スクリプトにまとめたもの。6-3 の最小ループに対して、次の2つを足している。

  - 6-4: while True を for step in range(MAX_STEPS) に置き換え、上限で必ず止める。
         for ... else で「break せずに回り切った＝上限到達」を検知して警告する。
  - 6-5: 各ステップに print ログを仕込み、stop_reason・トークン・道具呼び出しを記録する。
