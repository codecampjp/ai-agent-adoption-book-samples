"""6-3 手書き実装：whileループでtool_useをさばく（最小エージェント）。

本書 6-3 節の掲載コードを、そのまま実行できる形にまとめたもの。
第4章の「一往復のツール使用」を while ループで包み、モデルが
「もう道具は要らない」と判断する（tool_use を返さなくなる）まで
自走させる、最小のエージェントループ。

ループの節目 (a)〜(d) は、本文 6-3 のコードコメントと対応する。
...
実行:
    ...
    ANTHROPIC_API_KEY=sk-... python 6-3_agent_loop_minimal.py   # 本番
