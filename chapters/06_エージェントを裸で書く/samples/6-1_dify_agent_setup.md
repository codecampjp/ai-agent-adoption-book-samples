...
> **スクリーンショットについて（撮影メモ）**
> 画像は `images/6-1_setup/` に置く。以下の `![PLACEHOLDER: ...]` は撮影後に差し替える。
> 撮影時の共通条件：
> - 日本語UI、ブラウザ幅1280px程度、ズーム100%
> - ワークスペース名・アカウント名・メールアドレス・APIキーはダミー化またはマスク
> - アプリ名は「社内ドキュメントQA」、ナレッジ名は「サンプル社内規程」で統一
> - 注目箇所（ボタン・ラベル）は赤枠等で囲まず素で撮る（強調は掲載時に別途指定）

## 0. 前提

**Dify環境**（どちらか一方）

- クラウド版：https://cloud.dify.ai にサインアップ（無料枠で本手順は完結する）
- セルフホスト版：
...
![PLACEHOLDER: 01_model_provider.png](images/6-1_setup/01_model_provider.png)
...
4. ステータスが「利用可能」になるまで待つ（通常1分以内）

![PLACEHOLDER: 02_knowledge_upload.png](images/6-1_setup/02_knowledge_upload.png)

> **スクショ02**：ナレッジ作成画面。`6-1_サンプル社内規程.md` をアップロードした直後（ファイル名が表示され、チャンク設定がデフォルトのまま）の状態。見せたいこと＝「ファイルを放り込むだけでRAGの検索対象になる」。

![PLACEHOLDER: 03_knowledge_ready.png](images/6-1_setup/03_knowledge_ready.png)
...
![PLACEHOLDER: 04_marketplace_wikipedia.png](images/6-1_setup/04_marketplace_wikipedia.png)
...
![PLACEHOLDER: 05_create_agent_app.png](images/6-1_setup/05_create_agent_app.png)
...
![PLACEHOLDER: 06_agent_config_overview.png](images/6-1_setup/06_agent_config_overview.png)
...
![PLACEHOLDER: 07_agent_mode_react_label.png](images/6-1_setup/07_agent_mode_react_label.png)
...
![PLACEHOLDER: 08_run_q1_single_tool.png](images/6-1_setup/08_run_q1_single_tool.png)
...
![PLACEHOLDER: 09_run_q2_two_tools_trajectory.png](images/6-1_setup/09_run_q2_two_tools_trajectory.png)
...
> **【本文図版を兼ねる】** この1枚は書籍本文の**図6-1-2**としてそのまま掲載する（`images/図6-1-2_実行ログの足取り.png`）。撮影要件の詳細（写っているべき4要素・注意点）は章フォルダの `図表一覧.md`「図6-1-2」を正とし、撮影は1回で両方に流用する。
...
![PLACEHOLDER: 10_logs_view.png](images/6-1_setup/10_logs_view.png)
...
| # | ファイル名 | 内容 | 優先度 |
|---|-----------|------|--------|
| 01 | `01_model_provider.png` | モデルプロバイダー設定済み | 低（文章で足りる） |
| 02 | `02_knowledge_upload.png` | ナレッジへのファイルアップロード | 中 |
| 03 | `03_knowledge_ready.png` | ナレッジ「利用可能」 | 低（02と統合可） |
...
| 09 | `09_run_q2_two_tools_trajectory.png` | 質問2（ナレッジ→Wikipedia）の足取り | **最高**（本文図6-1-2を兼ねる。要件は図表一覧.mdが正） |
