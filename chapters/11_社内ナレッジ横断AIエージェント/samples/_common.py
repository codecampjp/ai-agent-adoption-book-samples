"""第11章サンプル共通モジュール ── ダミー社内コーパスと検索・権限の部品

APIキー・ネットワーク不要で章のコードを確かめられるよう、社内文書を模した
小さなコーパスと、ハイブリッド検索（擬似ベクトル＋キーワード）・RRF・リランキング・
権限判定を、外部依存なしの純Pythonで用意する。

本文（11-6/11-3/11-7）の骨格に対応する「動く実体」。実運用では、検索は各ソースの
自前インデックスを公開する自作MCPサーバー（11-5）に、権限判定は自社の認可基盤に
置き換わる。
"""
...
def can_view(user: dict, meta: dict) -> bool:
    """この利用者がこの文書を見てよいか（確定的な権限判定・11-3）。"""
