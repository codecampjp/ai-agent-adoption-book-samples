"""10-3 権限制御つきナレッジ横断エージェント ── 全体パイプライン（オフライン）

本文 10-3〜10-7 を1本につないだ完全版。検索Subgraph（10-2/10-5）→権限チェック＋
Conditional Edge（10-6）→回答生成＋引用付与（10-7）を LangGraph で組む。

APIキー・ネットワーク不要。回答生成は本物のLLMの代わりに「擬似モデル」
(_pseudo_model) を使う。本番では、この擬似モデルを実際のモデル呼び出しに、
検索を 10-4 の MCP 経由に置き換える（本文どおり）。
"""
...
def answer(state: State):
    ...
    # 答えに現れた番号を、対応表から実在のID・抜粋へ引き直す（でっち上げを排除）
