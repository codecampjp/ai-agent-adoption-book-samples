"""14-3 評価データセットの中身を見る（オフライン）

本文 14-3 の「入力＋期待する観点」の組を、そのまま表示する。正常系だけでなく
異常系（権限のない機密／存在しない情報）も含まれていることを確認する。
"""
...
def main():
    ...
    for case in DATASET:
        exp = case["expected"]
        kind = "異常系" if exp["must_refuse"] else "正常系"
