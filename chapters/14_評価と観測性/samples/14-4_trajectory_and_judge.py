"""14-4 Trajectory Eval と LLM-as-a-Judge（擬似ジャッジ）でスコアを出す（オフライン）

本文 14-4 の2つの評価法を、擬似エージェント＋擬似ジャッジでオフライン再現する。
...
本番では、擬似ジャッジを評価基盤のLLM-as-a-Judgeやpromptfooの`llm-rubric`に置き換える。
"""
