"""12-2 本章で新しく使う機能の最小デモ ── 並列fan-out・reducerでのfan-in・Send

本文 12-2 で概念を押さえた「Supervisorパターンと並列実行」の土台になる3点を、
それぞれ独立した最小例で動かして確かめる。LLM もネットワークも不要。

(1) fan-out（複数行き先）＋reducerでのfan-in：1ノードから複数ノードへ同時に分岐し、
    並列に走らせて、結果を1つの状態にマージする。
(2) Send API：分岐数を実行時に決める動的な並列（map-reduce）。
(3) recursion_limit：暴走への安全弁。
"""
...
def dispatch(state: FanState):
    # 分岐関数がリストを返すと、3つのworkerが同じスーパーステップで並列実行される
...
def demo_fanout():
    ...
    for name, fn in [("worker_a", worker_a), ("worker_b", worker_b),
        ...
        b.add_node(name, fn)
    # START から3ノードへ fan-out（複数行き先）
    b.add_conditional_edges(START, dispatch, ["worker_a", "worker_b", "worker_c"])
    # 3ノードから gather へ集約すると、LangGraph が全ブランチの完了を自動で待つ
...
def demo_recursion_limit():
    ...
    b.add_node("step", lambda s: {"n": s["n"] + 1})
    b.add_edge(START, "step")
    b.add_conditional_edges("step", lambda s: "step" if s["n"] < 3 else END,
                            {"step": "step", END: END})
    out = b.compile().invoke({"n": 0}, {"recursion_limit": 10})
    print("(3) recursion_limit: n =", out["n"], "（上限超過は GraphRecursionError で停止）")
