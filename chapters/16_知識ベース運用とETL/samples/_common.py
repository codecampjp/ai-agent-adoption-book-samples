"""第16章サンプル共通モジュール ── 再埋め込みコストの試算（擬似）

APIキー・ネットワーク・外部ライブラリ不要で、本章16-6「再埋め込みの戦略とコスト試算」を
手元で回せるよう、次を純Python（標準ライブラリのみ）で用意する。

  ・全再構築コスト … コーパス全体を作り直す（埋め込みモデル変更時＝本文 16-6）
  ・差分更新コスト … 変わったチャンクだけ作り直す（文書更新時＝本文 16-6）
...
実額は変動が速いので、必ず使う時点の公式ドキュメントで確認すること（本文 16-6）。
"""
...
# 実額ではない。構造（チャンク数×1チャンクのトークン×単価でコストが決まる）だけ見る。
...
def choose_strategy(document_changed: bool, model_changed: bool) -> str:
    """「何が変わったか」から再埋め込み戦略を選ぶ（本文 16-6 の意思決定フロー）。
