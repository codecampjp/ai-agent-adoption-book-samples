"""16-6 再埋め込みの戦略とコスト試算 ── 全再構築 vs 差分（オフライン）
...
def main():
    ...
    # 1) 「何が変わったか」で戦略を選ぶ（本文 16-6 の意思決定フロー）
    print("=== 何が変わったか → 戦略 ===")
    cases = [
        ("文書だけ更新", True, False),
    ...
    print("※ 単価・チャンク数・変更率は擬似値。本番は実際の埋め込み料金と自社の実データに置き換える（本文 16-6）。")
    print("※ 日常の文書更新は差分で回し、全再構築は埋め込みモデル乗り換え時に限る（本文 16-6）。")
    print("※ 差分を効かせるには『何が変わったか』を知る仕組み（更新日時・ハッシュ・CDC＝16-4）が要る。")
